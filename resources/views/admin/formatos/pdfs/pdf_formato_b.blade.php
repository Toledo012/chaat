...
<head>
    ...
    <style>
        ...
        .header { border-bottom: 2px solid #000; margin-bottom: 8px; padding-bottom: 6px; }
        .header img { width: 90px; }
        .titulo { text-align:center; font-weight:bold; font-size:13px; margin:6px 0; text-transform:uppercase; }
        .section { background-color:#d1e7dd; font-weight:bold; padding:4px; margin-top:6px; margin-bottom:2px; font-size:11px; }
        table { width:100%; border-collapse:collapse; margin-bottom:4px; }
        th, td { border:1px solid #000; padding:4px; vertical-align:top; }
        th { background-color:#f2f2f2; width:18%; }
        .texto { min-height:22px; }
        .firmas td { border:none; text-align:center; padding-top:20px; }
        .footer { border-top:1px solid #999; text-align:center; font-size:10px; margin-top:6px; padding-top:4px; }
    </style>
</head>
<body>
...
<table>
    ...
    <tr>
        <th>No. Inventario</th><td>{{ $servicio->numero_inventario }}</td>
        <th>Tipo Servicio</th><td>{{ $servicio->tipo_servicio }}</td>
    </tr>
    <tr>
        <th>Origen Falla</th><td>{{ $servicio->origen_falla }}</td>
        <td colspan="2"></td>
    </tr>
</table>
...
<table class="firmas" width="100%">
    <tr>
        <td width="33%"><strong>Usuario</strong><br><br>{{ $servicio->firma_usuario ?? '___________________' }}</td>
        <td width="33%"><strong>Responsable</strong><br><br>{{ $servicio->firma_tecnico ?? '___________________' }}</td>
        <td width="33%"><strong>Jefe de Área</strong><br><br>{{ $servicio->firma_jefe_area ?? '___________________' }}</td>
    </tr>
</table>

<div class="footer">Sistema de Formatos Digitales — SEMAHN</div>
</body>
</html>
